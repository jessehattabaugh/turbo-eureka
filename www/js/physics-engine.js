...
export class PhysicsEngine {
  ...
  init() {
    ...
    // Create renderer with optimized settings for pointer events
  ...
  getBodyAtPoint(point) {
    ...
    for (const body of allBodies) {
      if (!body.isStatic && !this.bodies.dynamic.includes(body) && this.isPointInBody(body, point)) {
        return body;
      }
    }
  ...
  resize(width, height) {
    ...
    const ground = this.bodies.static.find(body => {return body.label === 'ground'});
    if (ground) {
    ...
    const leftWall = this.bodies.static.find(body => {return body.label === 'leftWall'});
    const rightWall = this.bodies.static.find(body => {return body.label === 'rightWall'});
